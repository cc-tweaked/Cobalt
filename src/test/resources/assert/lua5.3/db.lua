...
function f(a,b)
  ...
  assert(x == a and y == b)
  print(">", debug.setlocal(2, 3, "pera"), "<") assert(debug.setlocal(2, 3, "pera") == "AA".."AA")
...
assert(a == "x" and b == 10)
assert(not debug.getlocal(co, 1, 6)) -- should be 5, but we do it wrong!
assert(debug.setlocal(co, 1, 1, 30) == "x")
assert(not debug.setlocal(co, 1, 6, 40)) -- again, should be 5
...
t = {"'yield'", "'f'", "in function <"} -- should be 'coroutine.yield'
